# Slopsmith Demucs Server Constitution
...
Slopsmith itself can live anywhere (NAS, Docker host, laptop). This split
...
Slopsmith plugins SHOULD be added as new endpoints in this server before
...
`/jobs/{job_id}`) so the Slopsmith UI can show real progress instead of
spinning.
